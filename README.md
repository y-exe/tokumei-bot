...
* **豊富な管理者機能:** 投稿者の特定、利用制限（BAN）、フィルタリング強度の調整など、管理者が柔軟に運用できるツール群。
...
* `/id [メッセージID or URL]`: 指定された匿名メッセージの投稿を特定します。
* `/state [ユーザーID]`: ユーザーのBAN状態を確認・変更します。
...
* `/log channel [チャンネル]`: レポートの送信先を設定します。

## 導入と初期設定

1. **Botをサーバーに招待:** 必要権限（メッセージ送信・管理、Webhook管理等）を付与して招待します。
2. **チャンネルを設定:** 用途に合わせて `!set` または `!setyoubou` を実行してください。
3. **レポートチャンネルの設定:** 管理者のみ閲覧可能なチャンネルで `/log channel` を実行し、通報の宛先を指定します。
